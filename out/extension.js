...
const notionPagesProvider_1 = require("./providers/notionPagesProvider");
function activate(context) {
    ...
    const notionPagesProvider = new notionPagesProvider_1.NotionPagesProvider(notionService);
...
async function importNotionPage(notionService) {
...
async function editWithClaude(claudeService) {
    ...
    try {
        await vscode.window.withProgress({
            ...
            const choice = await vscode.window.showInformationMessage('Claude has modified your content. Review the changes.', 'Show Diff', 'Accept', 'Reject');
            if (choice === 'Show Diff') {
...
async function showDiff(original, modified, title) {
...
async function applyEdit(editor, selection, newContent, isSelection) {
...
async function exportToNotion(notionService) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('No active editor found');
        return;
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (pageId) {
                ...
                await notionService.updatePage(pageId, content);
